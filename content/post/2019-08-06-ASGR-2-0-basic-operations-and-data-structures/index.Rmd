...
As the third post in the Scientist's Guide to R series (click [here](https://craig.rbind.io/post/2019-05-17-asgr-basic-workflow/) for the 1st post), we advance to the brink of the next major stage of the data analysis with R process: cleaning and transforming data. However, before we can clean or transform anything, we will need to know how to perform a few basic operations in R and introduce you to some common data structures in R, which are the topics of this post. 

# **Introduction**

You've managed to import your data and are wondering what to do next? The way in which you *should* proceed will critically depend upon the structure of your data (e.g. variable types, organization, etc.), but before we get there you need to know which sorts of things you *can* do in R. Accordingly, this post will introduce you to some basic operations and data structures in R, followed by progressively more advanced operations moving forward. There is a lot of content here so feel free to skip sections you are already familiar with. If this is all new you should take the time to read through it since we will continue building upon this foundation as we progress to more advanced topics. For example, you need to understand how data are represented by R before you start manipulating those representations.

# **Basic Calculations**

Like any decent data analysis software, R can perform an array of common mathematical and logical operations, as simply as you probably expect them to be.
```{r, eval = F}
#Basic calculations#####
5+6 #addition
...
To save some values (or virtually anything else) to use again later you assign them to a variable. This can be done in R using either "<-" or "=", but "<-" is typically recommended since "=" is also used to specify function arguments. See [here](https://stackoverflow.com/questions/1741820/what-are-the-differences-between-and-in-r) for more info if you're interested in the subtleties, e.g. "=" sometimes fails when you wouldn't expect it to, but "<-" typically does not. R studio also provides a handy keyboard shortcut for inserting "<-": [Alt] & [-]
```{r, eval = T}
...
```{r, eval = F}
#the only naming conventions are that a variable name should not begin with a
...
Most of the other things you will do in R operate on stored values or objects in a variety of forms. 
...
* [Data Frames](https://www.rdocumentation.org/packages/base/versions/3.6.1/topics/data.frame) = Also a 2D array with but column vectors can be different classes. These are the most common type of data structure in R. 
...
* [Lists](https://www.rdocumentation.org/packages/base/versions/3.6.1/topics/list): a list of arbitrary objects, more flexible than data frames but slightly less intuitive to work with. Lists elements can consist of multiple different data types, including the output of statistical tests and even entire data frames. Lists are a slightly more advanced topic, so they won't be covered further until later on in the blog series.
...
#using modulus 2 (divide by 2 and return the remainder) can tell you about even
#and odd components of a numeric vector

sum((x %% 2) == 0) #how many even values are in x
sum((x %% 2) == 1) #how many odd values are in x

mean((x %% 2) == 1) #which proportion of x is odd
mean((x %% 2) == 0) #which proportion of x is even
...
#facilitates their representation and use as categorical variables. You can
#create a factor in R using either the factor() function, or by reclassifying
#another vector type using as.factor()
...
#involves constructing a set of 0/1 dummy variables for each level of the factor
#(but don't worry about this, because R will do it for you automatically)
...
#when creating a tibble, strings are not automatically converted to factors this
#is better from a data manipulation standpoint, which will be covered in a
...
df <- sample(1:200, 100, replace=T) #sample with replacement, save it in a vector called "df"
df
...
#quickly identifying data entry errors (to be demonstrated in a future post)

levels(as.factor(data$cyl)) #this is the same as unique but for factors. 
#It provides the added benefit of revealing what the order of factor levels is.

table(data$cyl) #show the unique values of a vector as well as the count of each
...
The functions you use in R operate within what is appropriately called the working environment, which consists of the functions and other objects that you have loaded in the current R session (this is what happens you load a package with the library function), as well as any variables, data objects, or functions you have created during the session.
...
The projects feature of R studio makes it much easier to keep your work organized, and using it is strongly recommended if you are working on anything that will take longer to complete than a single R session.

You can create/start a project using the ![projects](/2019-08-06-ASGR-2-0-basic-operations-and-data-structures/index_files/projects icon.png) menu in the top right corner of R studio, directly below the minimize/maximize/exit buttons. Creating a package and then returning to it later also sets the working directory to the project folder automatically. 


# **useful keyboard shortcuts (for R studio users)**

One of the great benefits of using R studio is that you can take advantage of keyboard shortcuts that make the coding process easier/faster. Here is a list of some shortcuts that I've found to be useful:
...
* reflow comments: highlight/select comments that extend beyond the viewable area of the screen or multiple lines and use [Ctrl] + [Shift] + [/] to reflow/wrap them for easier reading
...
* A link to the next post (data manipulation part 1) will be added in the near future.
